...
class Common extends Functions {
      ...
      public function assignActionPageLocation() {
        $url = $_SERVER['REQUEST_URI'];
        $url = explode('/', $url);
        $size = sizeof($url);
        if($size > 5) {
        	$actionpath = '../actions.php';
          $currentpage = './admin/companyregistration.php';
          $usertype = 'admin';
        }else{
        	$actionpath = './actions.php';
          $currentpage = './companyregistration.php';
          $usertype = 'user';
        }
        return array($actionpath,$size,$currentpage,$usertype);
      }
