<?php
  ...
  $alltypes = $functions->getAllCompanyTypesByStatusAndOrder();
  $alldepartments = $functions->getApprovedDepartments();
?>
		  		...
		  		<div class="col-md-12">
                    ...
                    <form action="../actions.php?action=addCompanyType" method="post">
                      <select class="form-control" name="departments" required="">
                        <option value="" >Please Select Work Type Department</option>
                      <?php foreach ($alldepartments as $depodatas){
                          $depodataid = $depodatas['id'];
                          $depodataname = $depodatas['name'];
                        ?>
                        <option value="<?=$depodataid?>"><?=$depodataname?></option>
                      <?php } ?>
                      </select>
                      <br>
                        <input required type="text" name="type" placeholder="Enter Works Type" class="form-control">
